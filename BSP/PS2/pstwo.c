...
//	GPIOB->CRH|=0X33300000;//PB13、PB14、PB15 推挽输出
  //输入  DI->PB12
	//PB12设置成输入	默认下拉  
	GPIO_Config(RCC_APB2Periph_GPIOB, GPIOB,GPIO_Pin_12, GPIO_Mode_IPD);

  //  DO->PB13    CS->PB14  CLK->PB15
	//PB13、PB14、PB15 推挽输出   
	GPIO_Config(RCC_APB2Periph_GPIOB, GPIOB,GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15, GPIO_Mode_Out_PP);
}
...
void Select_PS2()
{
	switch(PS2_DataKey())
	{
		...
		case PSB_PAD_UP:
			Chassis_Algorithm_Model(20,0,0);
		break;
		...
		case PSB_PAD_LEFT:
			Chassis_Algorithm_Model(0,20,0);
		break;
		...
		case PSB_L1:
			Chassis_Algorithm_Model(0,0,-20);
